...
class QQuickFiberCryptoBusyIndicatorNode : public QQuickAnimatedNode
{
public:
    QQuickFiberCryptoBusyIndicatorNode(QQuickFiberCryptoBusyIndicator *item);
...
QQuickFiberCryptoBusyIndicatorNode::QQuickFiberCryptoBusyIndicatorNode(QQuickFiberCryptoBusyIndicator *item)
...
void QQuickFiberCryptoBusyIndicatorNode::updateCurrentTime(int time)
{
...
void QQuickFiberCryptoBusyIndicatorNode::sync(QQuickItem *item)
{
    QQuickFiberCryptoBusyIndicator *indicator = static_cast<QQuickFiberCryptoBusyIndicator *>(item);
...
QQuickFiberCryptoBusyIndicator::QQuickFiberCryptoBusyIndicator(QQuickItem *parent) :
    QQuickItem(parent)
{
    setFlag(ItemHasContents);
}

QColor QQuickFiberCryptoBusyIndicator::color() const
{
    return m_color;
}

void QQuickFiberCryptoBusyIndicator::setColor(QColor color)
{
    if (m_color == color)
        return;
...
bool QQuickFiberCryptoBusyIndicator::isRunning() const
{
    return isVisible();
}

void QQuickFiberCryptoBusyIndicator::setRunning(bool running)
{
    if (running)
        setVisible(true);
}

int QQuickFiberCryptoBusyIndicator::elapsed() const
{
    return m_elapsed;
}

void QQuickFiberCryptoBusyIndicator::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData &data)
{
...
QSGNode *QQuickFiberCryptoBusyIndicator::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *)
{
    QQuickFiberCryptoBusyIndicatorNode *node = static_cast<QQuickFiberCryptoBusyIndicatorNode *>(oldNode);
    if (isRunning() && width() > 0 && height() > 0) {
        if (!node) {
            node = new QQuickFiberCryptoBusyIndicatorNode(this);
            node->start();
        }
        node->sync(this);
    } else {

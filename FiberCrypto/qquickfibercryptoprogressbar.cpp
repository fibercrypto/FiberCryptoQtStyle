...
class QQuickFiberCryptoProgressBarNode : public QQuickAnimatedNode
{
public:
    QQuickFiberCryptoProgressBarNode(QQuickFiberCryptoProgressBar *item);
...
QQuickFiberCryptoProgressBarNode::QQuickFiberCryptoProgressBarNode(QQuickFiberCryptoProgressBar *item)
...
void QQuickFiberCryptoProgressBarNode::updateCurrentTime(int time)
{
...
void QQuickFiberCryptoProgressBarNode::sync(QQuickItem *item)
{
    QQuickFiberCryptoProgressBar *bar = static_cast<QQuickFiberCryptoProgressBar *>(item);
...
void QQuickFiberCryptoProgressBarNode::moveNode(QSGTransformNode *transformNode, const QRectF &geometry, qreal progress)
{
...
QQuickFiberCryptoProgressBar::QQuickFiberCryptoProgressBar(QQuickItem *parent)
    : QQuickItem(parent)
{
    setFlag(ItemHasContents);
}

QColor QQuickFiberCryptoProgressBar::color() const
{
    return m_color;
}

void QQuickFiberCryptoProgressBar::setColor(const QColor &color)
{
    if (color == m_color)
        return;
...
qreal QQuickFiberCryptoProgressBar::progress() const
{
    return m_progress;
}

void QQuickFiberCryptoProgressBar::setProgress(qreal progress)
{
...
bool QQuickFiberCryptoProgressBar::isIndeterminate() const
{
    return m_indeterminate;
}

void QQuickFiberCryptoProgressBar::setIndeterminate(bool indeterminate)
{
...
void QQuickFiberCryptoProgressBar::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData &data)
{
...
QSGNode *QQuickFiberCryptoProgressBar::updatePaintNode(QSGNode *oldNode, QQuickItem::UpdatePaintNodeData *)
{
    QQuickFiberCryptoProgressBarNode *node = static_cast<QQuickFiberCryptoProgressBarNode *>(oldNode);
    if (isVisible() && width() > 0 && height() > 0) {
        if (!node)
            node = new QQuickFiberCryptoProgressBarNode(this);
        node->sync(this);
    } else {
        delete node;
        node = nullptr;
